...
if(isset($_POST["submite"])){
    ...
    $ddnValue = $_POST["ddn"];
    $idfonctionValue=$_POST["fonction"];

    if(empty($phoneValue) || empty($fnameValue) || empty($ddnValue)|| empty($lnameValue)|| empty($idfonctionValue)){

            $errorMessssage = "all fileds must be filed out!";

    }else{
       $succeseror="employe has been added";
    
    //include the client file
    include('employe.php');

    //create new instance of client class with the values of the inputs
    $client = new employe($fnameValue,$lnameValue,$phoneValue,$ddnValue,$idfonctionValue);

//call the insertClient method
$client->insertEmploye('employe',$connection->conn);

//give the $successMesage the value of the static $successMsg of the class
$successMessssage = employe::$successMsssg;

//give the $errorMesage the value of the static $errorMsg of the class
$errorMessssage = employe::$errorMsssg;
...
<?php
  ...
  ?> </div></div>
  <div class="col-xs-6">
							<a href="admi.php" class="btn btn-success" ><i class="material-icons">&#xE147;</i> <span>back</span></a>
						</div>
					</div>
				</div>
<div class="" >
